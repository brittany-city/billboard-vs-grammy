...
```{r}
grammy_music <- filter(grammy, (category %in% c("Record Of The Year","Album Of The Year","Song Of The Year")))
```
...
Time series example
```{r}
filteredMusic <- music_merge %>%
  filter(year == 2019) %>%
  filter(category == "Song Of The Year")
```

```{r}
qplot(x=filteredMusic$WeekID, y=filteredMusic$`Week Position`,
